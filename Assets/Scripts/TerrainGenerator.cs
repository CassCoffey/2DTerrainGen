...
public class TerrainGenerator : MonoBehaviour
{
    ...
    public GameObject Generate()
    {
        if (keys == null)
        {
            ...
            keys[0] = new Vector3((-deltaXMin)/2, 0);
        ...
        terrain.GetComponent<PolygonCollider2D>().sharedMaterial = material;
    ...
    private Mesh GenerateMesh(int start)
    {
        ...
        {
            mesh.subMeshCount = 2;
            Vector3[] verts = new Vector3[(segmentsPerSlope + 1) * 3];
            ...
            float uvSeg = (float)segmentsPerSlope / (float)textureWidth;
            float segmentWidth = (keys[start + 1].x - keys[start].x) / (float)segmentsPerSlope;
            float yMid = (keys[start + 1].y + keys[start].y) / 2f;
            float amplitude = (keys[start].y - keys[start + 1].y) / 2f;
            int vertIndex = 0;

            // Set up Vertices and UV vectors
            for (int i = 0; i < segmentsPerSlope + 1; i++)
            {
        ...
        {
            mesh.subMeshCount = 1;
            Vector3[] verts = new Vector3[(segmentsPerSlope + 1) * 2];
            Vector2[] uv = new Vector2[verts.Length];
    ...
    public void Update()
    {
        float segmentWidth = deltaXMin / (float)segmentsPerSlope;
        float deltaX = mainCamera.transform.position.x - previousLocation.x;

        segmentsMoved += deltaX / segmentWidth;

        if (mainCamera.transform.position.x >= keys[0].x + ((keys[1].x - keys[0].x)/2))
        {
            terrainList.Add(Generate());
            if (terrainList.Count >= 6)
            {
    ...
    //    {
    //        terrain.GetComponent<MeshFilter>().mesh = GenerateMesh(1);
    //        terrain.GetComponent<PolygonCollider2D>().points = Generate2DCollision(terrain.GetComponent<MeshFilter>().mesh);
    //    }
